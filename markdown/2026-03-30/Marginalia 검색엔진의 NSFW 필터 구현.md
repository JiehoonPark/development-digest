---
title: "Marginalia 검색엔진의 NSFW 필터 구현"
...
> [!info] 원문
> [An NSFW filter for Marginalia search](https://www.marginalia.nu/log/a_134_nsfw/) · Hacker News (Top)

## 핵심 개념

> [!abstract]
> Marginalia Search 개발자가 검색 결과의 성인 콘텐츠를 필터링하기 위해 단일 은닉층 신경망을 구현했습니다. fasttext와 qwen3.5를 활용해 10,000개의 샘플 데이터셋을 자동 레이블링한 후 빠른 CPU 기반 분류기를 훈련했습니다.

## 상세 내용

- 트랜스포머 모델 대신 단순 신경망으로 CPU 성능과 분류 정확도 균형 달성
- LLM을 활용한 자동 데이터 레이블링으로 수동 작업 회피, ollama와 qwen3.5로 구성된 오픈소스 파이프라인 사용

> [!tip] 왜 중요한가
> 검색엔진이나 콘텐츠 플랫폼 개발자에게 제한된 리소스로 효과적인 콘텐츠 필터링을 구현하는 실용적 접근법을 보여줍니다.
...
1. 검색 쿼리 실행
2. 결과를 ollama/Qwen 3.5로 전달하면서 NSFW 분류 지시 제공
3. 결과를 SAFE 또는 NSFW로 라벨링
...
- [원문 링크](https://www.marginalia.nu/log/a_134_nsfw/)
- via Hacker News (Top)
- engagement: 86
